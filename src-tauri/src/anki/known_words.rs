...
/// someone else's deck.
///
/// `prop:ivl` is the maturity test, and it belongs in the query rather than in a
/// filter over the results: Anki applies it inside the collection, so an immature
/// note is never fetched at all. A word sitting in a deck is not a word you know —
/// one added yesterday and failed since would otherwise count exactly as much as
/// one you have held for a year, and every sentence would rank as too easy.
///
/// A note is mature once *any* of its cards is, which is the right reading for a
/// word whose recognition card is old and whose recall card was added last week:
/// the word is known, only one direction of it is still being learned.
...
/// Reads the settings that decide what the index will contain, under one lock, so a
/// refresh cannot pair the sources from before an edit with the threshold from
/// after it — and so the index records exactly what it was built under.
...
/// Clears the index from memory and removes its file, so a Refresh that finds
/// nothing to build leaves no cache for the next launch to restore. The file
/// removal is best-effort: a leftover file would be read back as `stale` against
/// the (now empty) sources and reported off anyway, so a failure here is logged,
/// not fatal.
...
/// Stores the freshly built index and writes it to disk. The write is best-effort:
/// the in-memory index already serves this session, so a disk failure costs only
/// the restore on the NEXT launch — worth a warning, not worth failing the Refresh
...
pub(crate) fn refresh_known_words_inner<R: Runtime>(
    ...
    if words.is_empty() {
        ...
        return Ok(KnownWordsSnapshot {
            status: "empty".into(),
            // Naming the threshold matters more than it looks. Since only settled
            // words count, someone a fortnight into a new deck gets nothing back —
            // and "check your fields hold text" would send them to fix a setting
            // that was never wrong. The honest answer is that they are early.
            message: format!(
    ...
    store_and_persist_known_words(
        ...
        KnownWordIndex {
            words,
            built_at_ms,
            // Recorded so a later change to the sources OR the threshold is
            // detectable as a stale index on the next launch rather than silently
            // trusted. Stored whole, so nothing that shaped this list is left out.
            build: build.clone(),
        },
    )?;
